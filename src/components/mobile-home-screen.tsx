...
function AppIcon({
  ...
  return (
    ...
    >
      <img src={app.icon} alt="" className="h-12 w-12 drop-shadow-[0_6px_14px_rgba(0,0,0,0.35)]" />
      <span className="text-[11px] font-medium text-white drop-shadow-sm">{app.label}</span>
    </button>
  );
}

// Faixa só com a logo (a mesma usada em todo o resto do site) — tamanho
// 4x1: largura inteira, altura de 1 ícone só. Fundo preto translúcido (em
// vez do vidro claro dos outros widgets) — pedido explícito, só pra esse.
// `max-h-full` (não `h-full`) no <img> porque percentual de altura só
// resolve direito se TODA a cadeia de pais tiver altura definida.
function LogoBanner() {
  return (
    <div className="flex h-full min-h-0 items-center justify-center rounded-2xl border border-white/10 bg-black/45 px-6 shadow-float backdrop-blur-xl">
      <img src={logo} alt="Olha o Take" className="max-h-[35%] w-auto object-contain" />
    </div>
  );
}

// Grade de verdade (CSS Grid, 4 colunas × 7 linhas de altura IGUAL — 1fr
// cada): logo 4x1 no topo, vídeo 3x5 ao lado de mais 5 ícones, e uma
// última linha com 4 ícones. `h-svh` no container (altura FIXA da tela,
// não só mínima) + `min-h-0` em cada célula é o que faz o grid realmente
// dividir em frações iguais — sem `min-h-0`, o conteúdo grande
// (imagem/vídeo) força a célula a crescer além do 1fr ("grid blowout", um
// problema clássico do CSS Grid).
export function MobileHomeScreen({
  ...
  return (
    <div
      className="mx-auto grid h-svh w-full max-w-xs grid-cols-4 gap-3 overflow-hidden px-6 pt-[max(env(safe-area-inset-top),3.5rem)] pb-4"
      style={{ gridTemplateRows: "repeat(7, 1fr)" }}
    >
      <div style={{ gridColumn: "1 / span 4", gridRow: "1" }} className="min-h-0">
        <LogoBanner />
      </div>

      <div style={{ gridColumn: "1 / span 3", gridRow: "2 / span 5" }} className="min-h-0">
        <ReelWidgetFill />
      </div>
      ...
      {/* Spans as 5 linhas do vídeo como UMA célula só; por dentro, 5 divs
          ...
          vídeo. */}
      <div style={{ gridColumn: "4", gridRow: "2 / span 5" }} className="flex min-h-0 flex-col gap-3">
        {SIDE_ICONS.map((app) => (
      ...
      <div style={{ gridColumn: "1 / span 4", gridRow: "7" }} className="flex min-h-0 items-center justify-between">
